...
import RecipientGroupView from "@/components/Model/RecipientGroupView";
...
const RecipientGroup: React.FC<TablePageProps> = () => {
    const [state, setState] = useState<stateProps[]>([]);
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const router = useRouter();
    const [stateView, setStateView] = useState<stateProps>({
        recipientGroupID: '',
        name: '',
        recipientList: '',
        tag: ''
    ...
    const fetchleadsData = async () => {
        ...
        try {
            await axios
                .get("/api/recipientGroup", { headers: { Authorization: "Bearer " + token } })
                .then((result) => {
    ...
    const handleDelete = async (item?: any) => {
        if (confirm("Are sure you want to Delete Record?")) {
            await axios.delete("/api/recipientGroup?id=" + item.recipientGroupID,
            ...
            <div className="flex flex-col gap-10">
                ...
                <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                    ...
                    <div className="max-w-full overflow-x-auto">
                                        ...
                                        <tr key={key}>
                                            ...
                                            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                                <div className="flex items-center space-x-3.5">
                                                    <button className="hover:text-primary" onClick={() => handleView(item)} >
                                                        <svg
                        ...
                        <RecipientGroupView isOpen={isOpen} setIsOpen={setIsOpen} openModal={openModal} closeModal={closeModal} stateView={stateView} setStateView={setStateView} fetchleadsData={fetchleadsData} />
                    </div>
                </div>
            </div>
